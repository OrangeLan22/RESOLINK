      ...
      <div class="tools-container flex">
        <div class="top-tools">
          ...
          <div class="search-container">
            <input type="text" v-model="searchKeyword" placeholder="通过空间名称查找">
            <button>搜索</button>
          </div>
          ...
          <div v-else class="resources-cards">
            <div class="resource-card" v-for="resource in resources" :key="resource.id">
              ...
              <div v-if="resource.max_available_minutes" class="available-time">
                <span>可预约时长：{{ resource.stage === 0 ? formatDuration(resource.max_available_minutes) : '0' }}</span>
              </div>
...
import SpaceReservation from './SpaceBooking/SpaceReservation.vue';
...
const aiMessages = ref([
  {
    type: 'ai',
    content: '您好！请问您需要什么样的会议室？我可以根据您的需求帮您推荐合适的空间资源。'
  }
])
...
const sendAIMessage = () => {
  ...
  setTimeout(() => {
    aiMessages.value.push({
      type: 'ai',
      content: '收到您的消息！目前AI功能正在开发中，暂时无法提供智能回复。您可以手动浏览和选择适合的会议室。'
    })
