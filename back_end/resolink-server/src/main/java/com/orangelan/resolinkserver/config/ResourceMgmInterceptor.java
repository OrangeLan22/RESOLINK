...
/**
 * 资源管理权限验证拦截器
 * 用于验证访问/resource-mgm路径的请求是否具有权限
 */
@Component
public class ResourceMgmInterceptor implements HandlerInterceptor {
    
    private static final Logger logger = LoggerFactory.getLogger(ResourceMgmInterceptor.class);
    private final AuthorityRepository authorityRepository;
    private final JwtTokenUtil jwtTokenUtil;
    
    private final ObjectMapper objectMapper;
    
    public ResourceMgmInterceptor(AuthorityRepository authorityRepository, JwtTokenUtil jwtTokenUtil) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 检查resource-mgm权限是否为1
        if (authority.getResourceMgm() != 1) {
            logger.error("没有操作权限，resource-mgm权限：{}", authority.getResourceMgm());
