...
/**
 * 公共信息权限验证拦截器
 * 用于验证访问/public-info路径的请求是否具有权限
 */
@Component
public class PublicInfoInterceptor implements HandlerInterceptor {
    
    private static final Logger logger = LoggerFactory.getLogger(PublicInfoInterceptor.class);
    private final AuthorityRepository authorityRepository;
    private final JwtTokenUtil jwtTokenUtil;
    
    private final ObjectMapper objectMapper;
    
    public PublicInfoInterceptor(AuthorityRepository authorityRepository, JwtTokenUtil jwtTokenUtil) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 检查public-info权限是否为1
        if (authority.getPublicInfo() != 1) {
            logger.error("没有操作权限，public-info权限：{}", authority.getPublicInfo());
