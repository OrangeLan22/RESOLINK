...
/**
 * 账号管理权限验证拦截器
 * 用于验证访问/account-mgm路径的请求是否具有权限
 */
@Component
public class AccountMgmInterceptor implements HandlerInterceptor {
    
    private static final Logger logger = LoggerFactory.getLogger(AccountMgmInterceptor.class);
    private final AuthorityRepository authorityRepository;
    private final JwtTokenUtil jwtTokenUtil;
    
    private final ObjectMapper objectMapper;
    
    public AccountMgmInterceptor(AuthorityRepository authorityRepository, JwtTokenUtil jwtTokenUtil) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 检查account-mgm权限是否为1
        if (authority.getAccountMgm() != 1) {
            logger.error("没有操作权限，account-mgm权限：{}", authority.getAccountMgm());
