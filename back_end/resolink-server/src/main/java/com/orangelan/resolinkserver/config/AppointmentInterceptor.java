...
/**
 * 预约管理权限验证拦截器
 * 用于验证访问/appointment路径的请求是否具有权限
 */
@Component
public class AppointmentInterceptor implements HandlerInterceptor {
    
    private static final Logger logger = LoggerFactory.getLogger(AppointmentInterceptor.class);
    private final AuthorityRepository authorityRepository;
    private final JwtTokenUtil jwtTokenUtil;
    
    private final ObjectMapper objectMapper;
    
    public AppointmentInterceptor(AuthorityRepository authorityRepository, JwtTokenUtil jwtTokenUtil) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 检查appointment权限是否为1
        if (authority.getAppointment() != 1) {
            logger.error("没有操作权限，appointment权限：{}", authority.getAppointment());
